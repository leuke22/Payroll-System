...
public class A_HomePageStaff extends JPanel {
	...
	public A_HomePageStaff() {
		...
		lblNewLabel.setBounds(170, 273, 631, 38);
		add(lblNewLabel);
		...
		comboBox.setBounds(811, 291, 115, 22);
	...
	private void filltable(String selectedMonth) {
	    try {
	        Connection con = DriverManager.getConnection("jdbc:sqlite:src/Database/usersDatabase.db");
	        Statement state = con.createStatement();
	        ResultSet set = state.executeQuery("SELECT * FROM usersStatus WHERE Date LIKE '%" + selectedMonth + "%'");

	        tbModel = (DefaultTableModel) table.getModel();
	        tbModel.setRowCount(0); // Clear the existing table data
